<?php
/**
 * transactions.php
 * Admin transactions page for the MERKADO POS system.
 * Displays a filterable, paginated list of all transactions with summary cards.
 * Includes a modal for viewing individual transaction details.
 * Only accessible to users with the 'admin' role.
 */

// Start session and verify login
session_start();

// Redirect to login if not logged in
if(!isset($_SESSION['username'])){
    header("Location: ../../index.php");
    exit();
}

// Redirect to cashier page if user is not an admin
if($_SESSION['role'] != 'admin'){
    header("Location: ../../cashier/controllers/cashier.php");
    exit();
}

// Connect to database for cashier dropdown
include(__DIR__ . "/../../config/connect.php");
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Transactions - MERKADO Admin</title>
    <!-- Transaction page styles -->
    <link rel="stylesheet" href="../views/css/transactionstyle.css">
</head>
<body>
    
    <!-- Sidebar Navigation -->
    <div class="sidebar">
        <div class="sidebar-header">
            <img src="../images/merkado-icon.png" alt="MERKADO logo">
            <h2><a href="admin.php">MERKADO</a></h2>
        </div>
        <ul class="sidebar-links">
            <li><a href="dashboard.php">Dashboard</a></li>
            <li><a href="inventory.php">Inventory</a></li>
            <li><a href="transactions.php">Transactions</a></li>
            <li><a href="accounts.php">Accounts</a></li>
            <li><a href="../models/adminlogout.php">Log Out</a></li>
        </ul>
    </div>

    <!-- Main Content Area -->
    <div class="mainshift">
        <div class="header-section">
            <h1>Transactions</h1>
        </div>

        <!-- Filter Section — date range, cashier, payment method -->
        <div class="filter-section">
            <div class="filter-row">
                <div class="filter-group">
                    <label>From Date:</label>
                    <input type="date" id="dateFrom" class="filter-input">
                </div>
                <div class="filter-group">
                    <label>To Date:</label>
                    <input type="date" id="dateTo" class="filter-input">
                </div>
                <div class="filter-group">
                    <label>Cashier:</label>
                    <select id="filterCashier" class="filter-input">
                        <option value="">All Cashiers</option>
                        <!-- Dynamically render cashier options from database -->
                        <?php
                        $cashier_sql = "SELECT id, username FROM users WHERE role = 'cashier'";
                        $cashier_result = $conn->query($cashier_sql);
                        while($cashier = $cashier_result->fetch_assoc()){
                            echo "<option value='{$cashier['id']}'>{$cashier['username']}</option>";
                        }
                        ?>
                    </select>
                </div>
                <div class="filter-group">
                    <label>Payment Method:</label>
                    <select id="filterPayment" class="filter-input">
                        <option value="">All</option>
                        <option value="Cash">Cash</option>
                        <option value="E-Wallet">E-Wallet</option>
                        <option value="Debit/Credit">Debit/Credit</option>
                    </select>
                </div>
                <div class="filter-group">
                    <button id="searchBtn" class="search-btn">Search</button>
                    <button id="resetBtn" class="reset-btn">Reset</button>
                </div>
            </div>
        </div>

        <!-- Summary Cards — total transactions, sales, average -->
        <div class="summary-cards">
            <div class="summary-card">
                <h3>Total Transactions</h3>
                <p id="totalTransactions">0</p>
            </div>
            <div class="summary-card">
                <h3>Total Sales</h3>
                <p id="totalSales">₱0.00</p>
            </div>
            <div class="summary-card">
                <h3>Average Transaction</h3>
                <p id="avgTransaction">₱0.00</p>
            </div>
        </div>

        <!-- Transactions Table — populated via AJAX by transactionscript.js -->
        <div class="table-container">
            <table class="transactions-table">
                <thead>
                    <tr>
                        <th>Receipt #</th>
                        <th>Cashier</th>
                        <th>Customer</th>
                        <th>Total Amount</th>
                        <th>Payment Method</th>
                        <th>Date & Time</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody id="transactionsBody">
                    <tr><td colspan="7">Loading...</td><tr>
                </tbody>
            </table>
        </div>

        <!-- Pagination Buttons — generated by transactionscript.js -->
        <div class="pagination" id="pagination"></div>
    </div>

    <!-- View Transaction Detail Modal -->
    <div id="viewModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2>Transaction Details</h2>
                <span class="close">&times;</span>
            </div>
            <div id="modalBody"></div>
        </div>
    </div>

    <!-- External Scripts -->
    <script src="../../public/js/jquery-4.0.0.min.js"></script>
    <script src="../views/js/transactionscript.js"></script>
    <script src="../views/js/lowStockAlert.js"></script>
</body>
</html>